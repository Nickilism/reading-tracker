...
实现完成后用户评审要求调整，最终行为以 spec 末尾「评审修订记录」为准：

- Task 1 标题文案：去掉「· 全部年份」。
- Task 4 图表：柱色改为年度页配色轮换；顶部留白 + 数字标签防截断。
- 移除「高分榜 Top 10 封面区块」相关 Task（原 Task 3 的高分榜部分、Task 5 renderTop10 引用）；改为 Top10 筛选（并列纳入）。
- Task 8 首页入口：追加修复样式注入导致整页被入口遮挡的问题（覆盖仅限统计条）。
- 封面年份角标 hover 显示；评分类筛选默认评分降序并同步排序条高亮（round 3 调整）。
